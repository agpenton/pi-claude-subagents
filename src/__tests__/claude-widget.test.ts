...
describe('CLAUDE Theme Constants', () => {
  ...
  describe('BORDERS', () => {
    ...
    it('should have BRANCH space', () => {
      expect(CLAUDE.BORDERS.BRANCH).toContain(' ');
      expect(CLAUDE.BORDERS.BRANCH.length).toBeGreaterThanOrEqual(4);
    });
  });
  ...
  describe('color values validation', () => {
    ...
    it('should have consistent color palette', () => {
      const colors = CLAUDE.COLORS;
      expect(colors.success.startsWith('#4')).toBe(true);
      expect(colors.error.startsWith('#f')).toBe(true);
      expect(colors.warning.startsWith('#f')).toBe(true);
      expect(colors.accent.startsWith('#8')).toBe(true);
    });
  });

  describe('complete widget structure', () => {
    it('should have all required sections', () => {
      const requiredSections = ['SPINNER', 'BORDERS', 'ICONS', 'COLORS',
        'SPACING', 'TYPOGRAPHY', 'ANIMATION', 'WIDGET', 'AGENT', 'ACTIVITY'
      ];
    ...
    it('should have valid BRANCH spacing', () => {
      expect(CLAUDE.BORDERS.BRANCH.length).toBeGreaterThanOrEqual(4);
    });
  });
